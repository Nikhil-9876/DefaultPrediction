...
const { User, Banker } = require('../db/database');
...
// Route for banker login
router.post('/checkbanker', [
    body('email', 'Enter a valid email').isEmail(),
    body('password', 'Password cannot be blank').exists(),
], async (req, res) => {
    const errors = validationResult(req);
    ...
    try {
        const banker = await Banker.findOne({ email: req.body.email });
        if (!banker) {
            return res.status(401).json({ message: "No such account exists" });
        }

        const isPasswordValid = await bcrypt.compare(req.body.password, banker.password);
        if (!isPasswordValid) {
            return res.status(401).json({ message: "Invalid email or password" });
        }
